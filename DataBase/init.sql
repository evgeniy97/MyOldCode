CREATE TABLE `body` (
	`id_body` DECIMAL  PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`sex` BINARY,
	`surname` VARCHAR (255),
	`name` VARCHAR (255),
	'date_birth' DATE,
	`date_root` DATE,
	'death_date' DATE,
	`id_insurance` DECIMAL
);

CREATE TABLE `Insurance` (
	`id_insurance` DECIMAL  PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `Morgue` (
	`id_morgue` DECIMAL  PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`address` VARCHAR (255),
	`phone` VARCHAR (255)
);

CREATE TABLE `Pathologist` (
	`id_specialist` DECIMAL  PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`id_morgue` DECIMAL NOT NULL
...
CREATE TABLE `Diagnosis` (
	`id_diagnosis` DECIMAL  PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`name` VARCHAR(255) NOT NULL
);

CREATE TABLE `Map` (
	`id_body` DECIMAL NOT NULL,
	`id_diagnosis` DECIMAL NOT NULL,
	`id_specialist` DECIMAL NOT NULL
);
...
4) Вывести тела, которым поставил Diagnosisы определнный врач
5) Вывести все Diagnosisы конкретного тела
5) Вывести все тела с конкретным Diagnosisом
6) Вывести все тела с определленой страховой
7) Вывести все тела из опредленного Morgueа
8) Вывести всех врачей из определленого Morgueа
9) Добавить / Удаление карты
10) Из опредленного Morgueа определенным числом
11) Вывести все Diagnosisы которые ставил определенный врач 
...
16) Телефоны врачей которые ставили Diagnosisы определнному телу 
*/